...
**Prospectio** is a Python CLI automation system for B2B lead generation and outreach. It runs a 4-stage pipeline: collect leads from Google Maps → qualify them with Claude AI → approve via interactive terminal UI → send personalized emails via Gmail. Intended for self-hosted deployment on a Raspberry Pi 4B 8GB.
...
- **AI:** Anthropic SDK — `claude-sonnet-4-20250514` for lead scoring and email generation
...
- **`core/triagem.py`** — Sends each lead to Claude with a hardcoded customer profile (`PERFIL_LUIZ`). Returns score 1-10, classificacao (QUENTE/MORNO/FRIO), justificativa, dor_provavel, abordagem_sugerida, and recomendacao. Entry point: `triar_leads_pendentes()`.
...
- **`core/email_sender.py`** — Asks Claude to generate assunto + corpo_html + corpo_texto per lead, previews in terminal, confirms before sending. Entry point: `processar_fila_envio()`.
